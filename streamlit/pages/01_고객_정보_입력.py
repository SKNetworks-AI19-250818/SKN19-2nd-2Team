# pages/01_고객_정보_입력.py
import datetime as dt
import streamlit as st
from utils.state import init_state, go, NAV_COUNSELOR

st.set_page_config(page_title="고객 정보 입력", layout="wide")
init_state()

# ✅ date 세션 값이 비정상(str/None 등)일 경우 오늘 날짜로 교정
_date_val = st.session_state.get("date")
if not isinstance(_date_val, dt.date):
...
def try_login():
    ...
    # 콜백 안에서는 페이지 전환/rerun을 수행하지 않고, 플래그만 세팅
    st.session_state["_goto_counselor"] = True
...
# 콜백 밖의 정상 실행 경로에서 페이지 전환 수행
if st.session_state.pop("_goto_counselor", False):
