...
notebooks/
 ...
 └─ team/                  # 팀리뷰
    ...
    ├─ 03_ML모델비교.ipynb 
    ├─ 04_피처통합데이터.ipynb
    ├─ columns.json        # 활용 컬럼 관리      # 담당자별 피처코           # 전처리 패키징
    ├─ preprocess.py           # 모듈 테스트
    └─ modules/
        ├─ features_ksh.py
        ├─ features_mhs.py
        ├─ features_ohj.py
        ├─ features_pdy.py
        ├─ features_sangmin_korean.py
        └─ features_sangmin.py
...
설문 내용 분류:
1. 기본정보(나이, 연령, 성별)에 따른 금연 성공 상관 관계
2. 차상위 계층 등 가구조사에 따른 흡연율 상관 관계
3. 음주에 따른 금연 성공 상관 관계 
4. 신체 활동에 따른 금연 성공 상관 관계
5. 식생활 비만 및 체중에 따른 흡연 상관 관계
6. 흡연과 구강 건강 관계
7. 정신 건강과 흡연의 상관 관계 
8. 보건기관(금연 프로그램)과 금연 성공 상관 관계
9. 교육 수준에 따른 흡연율 상관 관계 
...
그래프 삽입 요망
운동·식습관: 걷기, 아침식사, 구강관리 등 규칙적인 생활습관이 있는 사람이 금연 성공률이 높음

그래프 삽입 요망
정신건강·수면: 상담의지나 규칙적인 수면습관이 있는 경우 성공률이 높음
...
anal_data.to_csv('analy_data_v2.csv', index=False, encoding='utf-8-sig')
```
